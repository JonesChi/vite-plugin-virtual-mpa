...
function createMpaPlugin$1(config) {
  ...
  return {
    ...
    configResolved(config2) {
      ...
      if (verbose) {
        const colorProcess = (path2) => vite.normalizePath(`${color.blue(`<${config2.build.outDir}>/`)}${color.green(path2)}`);
...
${inputFiles.join("\n")}`);
    ...
    configureServer(server) {
      ...
      const base = vite.normalizePath(`/${config2.base || "/"}/`);
      if (watchOptions) {
        const {
          events,
          handler,
        ...
        const isMatch = vite.createFilter(include || /.*/, excluded);
...
exports.createMpaPlugin = createMpaPlugin;
